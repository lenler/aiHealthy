...
# 可以写到简历上
项目：AI 减肥助手（核心模块开发）
基于 AI 大模型 Function Call 能力定制饮食解析 Tool，定义 Schema 强制结构化输出，解决 AI 解析数据不规范问题，解析准确率提升 90%+；
采用 SSE 流式传输实现 AI 数据实时返回，自研解析工具类完成分段 JSON 的拼接、校验与兜底，优化前端交互响应速度；
设计type多类型数据标识机制，实现自然语言 / 可编辑卡片的差异化渲染，支撑聊天页核心交互；
开发可编辑饮食卡片，实现热量实时计算、内容手动修改，通过全局状态管理完成与首页数据的双向同步，搭配本地缓存保证数据不丢失；
攻克流式解析、跨页同步、AI 输出规范等核心难点，实现模块从 0 到 1 落地，适配企业级数据交互与交互体验要求。