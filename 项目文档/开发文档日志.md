...
## 前端部分:
首页会展示用户的个人信息 包括用户昵称 头像 个人健康数据 然后就是两个表单 一个退出登录按钮
```tsx
return (
...
```
## 后端部分
路由部分很简单 获取用户健康信息 获取用户账号信息 根据用户健康信息的状态来添加或更新用户的健康信息 更新用户账号信息
```js
/**
 * 用户健康信息
 */
...
```
# 注册页面
页面的前端只是将复用了一个登录页面的样式 只是多添加了几个输入框而已\
```tsx
...
```
后端也很简单 就是根据用户输入的用户名判断用户是否存在 然后将用户的密码加密后存储到数据库
```js
Router.post('/',async (req,res)=>{
...
# 首页饮食建议开
## 整理思路
1. 从后端获取用户的健康信息 然后根据用户的健康信息发送给ai 获取饮食建议 顺便使用全局状态获取每日建议服用的总热量来替换掉之前的死数据
2. 获取用户信息后 将ai返回的建议渲染到首页 注意判断如果当前用户今天已经存在建议了 就不去向ai请求了 -->思路一: 将建议存储到状态中 如果存在就渲染到首页 如果不存在就发送后端请求
3. 经过思考 将ai处理逻辑放在后端 然后将判断逻辑放在前端 前端通过一个全局状态来判断是否存在建议 如果存在就渲染到首页 如果不存在就发送后端请求 这个状态可以直接定义为当日需要的卡路里 因为目标卡路里是包含在建议中的
- 后端 (Node.js) :
  - 修改 getOverview 接口或新增接口。
  - 集成 AI SDK (如 OpenAI/Gemini)。
  - 在数据库新增一张表 daily_advices (字段: user_id , date , content , target_calories )。
- 前端 (React) :
  - 状态层 : 在 src/store 中添加相关状态。
  - API层 : 在 src/api 中封装获取建议的请求。
  - UI层 :
    - 在 src/page/DashBoread/index.tsx 中，初始化时检查 Store。
    - 如果 Store 中没有今日数据，则发起请求，并在请求过程中展示 Loading 骨架屏 （这是你文档中提到的企业级规范点）。
    - 获取到数据后，更新 Store 和 UI
前端:
创建新的状态用于存储数据 
```ts
...
```
后续就是在首页中调用这个函数 然后根据状态中是否有数据来判断是否渲染到首页 就不多写了 
后端:
使用openaiskd 调用qwen模型 因为qwen模型支持模式化输出 所以我们可以直接将建议和目标卡路里返回给前端
```js
...
3. 优化用户交互体验 比如请求失败不会自动关闭弹
# 使用阿里云oss服务 实现用户头像的对象存储
...
## 整体业务逻辑
前端点击更换头像-->弹出模态框(文件上传组件)-->前端向后端申请凭证-->前端将数据上传到oss-->阿里云收到后返回图片地址-->
{
  -->前端将地址返回后端-->后端接受地址记录进数据库
  -->前端将头像切换成地址即可
}

开发任务: 
前端 
1. 结合antd图片上传组件与oss 设计模态框与上传组件 
2. 开发接口,将图片上传oss接口与将获取地址等信息传递给后端接口
3. 显示头像
后端:
1. 配合前段获取完整的鉴权体系 设计授权接口与获取前端发送信息接口
2. 修改数据库配置文件添加字段
3. 设计上传数据到数据库接口